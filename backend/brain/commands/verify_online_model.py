...
async def verify():
    ...
    try:
        ...
        print(
            f"📡 Sending test request via provider={provider_hint} to model={client.model} ({base_url})..."
        )
        ...
        print(f"\n🤖 Response: {content}")
        print(
            f"📊 Usage: {response['prompt_eval_count']} prompt tokens, {response['eval_count']} completion tokens"
        )
